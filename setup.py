...
def prompt_env(env_vars, force_default=False):
  ...
  env_vars["DOCKER_USER"] = input_with_default(
    "Enter Docker username",
    env_vars.get("DOCKER_USER") or default_user,
    force_default,
  )
  env_vars["DOCKER_USER_PASSWD"] = input_with_default(
    "Enter Docker user password",
    env_vars.get("DOCKER_USER_PASSWD") or default_user_password,
    force_default,
  )
  env_vars["DOCKER_GROUP"] = input_with_default(
    "Enter Docker group",
    env_vars.get("DOCKER_GROUP") or default_group,
    force_default,
  )
...
def build_docker_image(env_vars, debug_mode=False, no_cache=False, force_default=False):
  ...
  build_confirmation = input_with_default(
    f"Do you build image? (name: '{container_name}')", "y", force_default
  ).lower()
...
"""
    ...
    docker_build_command = [
      ...
      "build",
      "--build-arg",
      f"DOCKER_USER={env_vars['DOCKER_USER']}",
      "--build-arg",
      f"DOCKER_USER_PASSWD={env_vars['DOCKER_USER_PASSWD']}",
      "--build-arg",
      f"DOCKER_GROUP={env_vars['DOCKER_GROUP']}",
      "-f",
...
def run_docker_container_with_tty(container_name, env_vars, force_default=False):
  ...
  run_confirmation = input_with_default(
    f"Do you run docker container? (name: '{container_name})", "y", force_default
  ).lower()
...
def export_wsl(container_name, env_vars, force_default=False):
  export_confirmation = input_with_default(
    f"Do you make wsl file in order to import into wsl? (name: '{container_name}')",
    "y",
    force_default,
  ).lower()
  if export_confirmation == "y":
    ...
    export_path = input_with_default(
      "Enter output directory for wsl file", "current directory", force_default
    ).strip()
    ...
    try:
      subprocess.run(
        ["docker", "export", "-o", wsl_file_path, container_name], check=True
      )
    finally:
      stop_event.set()
    ...
    exec_confirmation = input_with_default(
      "Do you import wsl file into wsl? (for Windows)", "y", force_default
    ).lower()
    ...
    if rm_container_confirmation == "y":
      ...
      rm_image_confirmation = input_with_default(
        "Do you also delete current working image?", "y", force_default
      ).lower()
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(
    description="Script for setup of WSL environment with docker"
  )
  parser.add_argument(
    "-i",
    "--install",
    action="store_true",
    help="Install: Build & Export *.wsl & Import into WSL",
  )
  parser.add_argument(
    "-d",
    "--debug",
    action="store_true",
    help="Enable debug mode: Build & Run container & Enter it with tty",
  )
  parser.add_argument("--no-cache", action="store_true", help="Build with no cache")
  parser.add_argument(
    "-y", action="store_true", help="Response by default value for all prompts"
  )
  parser.add_argument(
    "--set-env",
    action="store_true",
    help="Set environment variable, saving into .env",
  )
  ...
  try:
    ...
    if len(sys.argv) == 1:
      ...
      env_vars = load_env()
      if env_vars:
        set_again_confirmation = input_with_default(
          "Detected .env in current directory.\nDo you try to set again? (require deleting .env)",
          "y",
          args.y,
        ).lower()
